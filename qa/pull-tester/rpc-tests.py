...
if 'ENABLE_WALLET' not in vars():
    ENABLE_WALLET=0
if 'ENABLE_DINGOCOIND' not in vars():
    ENABLE_DINGOCOIND=0
...
#Set env vars
if "DINGOCOIND" not in os.environ:
    os.environ["DINGOCOIND"] = BUILDDIR + '/src/dingocoind' + EXEEXT
...
if not (ENABLE_WALLET == 1 and ENABLE_UTILS == 1 and ENABLE_DINGOCOIND == 1):
    print("No rpc tests to run. Wallet, utils, and dingocoind must all be enabled")
    sys.exit(0)
...
class RPCTestHandler:
    ...
    def __init__(self, num_tests_parallel, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie dingocoinds, we can apply a
...
class RPCCoverage(object):
    ...
    commands per `dingocoin-cli help` (`rpc_interface.txt`).
