...
# Dependencies
ossPatchUrl="https://depends.dingocoincore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.dingocoincore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.dingocoincore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.dingocoincore.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
Standalone script to perform the gitian build of Dingocoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/dingocoin/dingocoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/dingocoin-binaries/${VERSION}/ \;
}
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/dingocoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean dingocoin git directory because of old caching
    if [ -d inputs/dingocoin/ ]; then
        echo "Cleaning Dingocoin directory..."
        rm -rf inputs/dingocoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit dingocoin="$COMMIT" --url dingocoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
